  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">寄件人資料</h1>
        <Link
          href="/senders/new"
              ...
              {senders.map((s) => (
                <tr key={s.id}>
                  <td className="px-4 py-3">
                    ...
                    {s.company && (
                      <p className="text-gray-400 text-xs">{s.company}</p>
                    )}
                  </td>
                  <td className="px-4 py-3 text-gray-600">
                    {s.address1}, {s.city} {s.postal}, {s.country}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right space-x-3">
                    ...
                    <DeleteButton
                      url={`/api/senders/${s.id}`}
                      confirmMessage={`確定要刪除寄件人「${s.name}」嗎？`}
                    />
                  </td>
                </tr>
              ))}
